...
PageRank передаёт значения рангов на границах partition. Поэтому рост числа MPI-процессов увеличивает не только параллелизм, но и объём коммуникации.
...
- `sync_time = 0`
- `sync_bytes = 0`
- основное время уходит на построение mining-графа, proxy-рёбра и global reduce
...
На данной платформе распределённое выполнение увеличивает накладные расходы быстрее, чем сокращает полное время запуска.

---

## Сравнение алгоритмов: деградация MPI=8

| Алгоритм / граф | MPI=1 | MPI=8 | Замедление |
|---|---:|---:|---:|
| BFS roadNet-PA | 2800 ms | 85824 ms | 30.6x |
| PageRank web-Google | 2805 ms | 17660 ms | 6.3x |
| SSSP web-Google | 398 ms | 9345 ms | 23.5x |
| TC web-Google | 1972 ms | 2356 ms | 1.2x |

Главный вывод: при выбранной конфигурации MPI=8 не даёт лучшего `total_time_exec` ни для одного алгоритма и графа.
...
Лучший `total_time_exec` для всех алгоритмов и графов даёт `MPI=1`.

Почему MPI не ускорил запуск:

- `roadNet-PA`: длинные пути -> много синхронизаций.
- `web-Google`: страницы-хабы -> копии вершин и больше обмена.
- `wiki-talk`: неравная нагрузка -> ожидание самого медленного процесса.
- TC: накладные расходы на построение mining-графа и proxy-рёбер.

Итог: на одном хосте накладные расходы MPI оказались больше выигрыша от распределения работы.
