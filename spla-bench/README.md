...
> **Курс / защита (LAGraph vs Spla, локальные датасеты):** пошаговая инструкция на русском — в корне репозитория курса: [`../README.md`](../README.md).
...
- PageRank (PR): Spla [`pr` example](https://github.com/SparseLinearAlgebra/spla) + LAGraph `gappagerank_demo` (in this course repo’s `config.py` / drivers)
