...
# Репозиторий курса (родитель spla-bench): локальные датасеты и Spla из корня
_COURSE_ROOT = ROOT.parent.resolve()
_GRAPHS_DATASETS = _COURSE_ROOT / 'graphs-theory-datasets'
_REPO_SPLA = _COURSE_ROOT / 'spla'

# Path to the directory, where datasets will be downloaded / лежат готовые .mtx
# [MUTABLE]
if _GRAPHS_DATASETS.is_dir():
    DATASET_FOLDER = _GRAPHS_DATASETS
else:
...
# Исходники Spla: сначала ../spla из корня курса, иначе копия в deps
_SPLA_SOURCES = _REPO_SPLA if _REPO_SPLA.is_dir() else (DEPS / 'spla')
_SPLA_BUILD = _SPLA_SOURCES / 'build'
...
TOOL_CONFIG: Dict[ToolName, ToolConfigurations] = {
    ...
    ToolName.spla: ToolConfigurations(
        sources=_SPLA_SOURCES,
        build=_SPLA_BUILD,
        # Текущий CMake Spla собирает примеры как bfs, sssp, tc (см. spla/CMakeLists.txt)
        algo_rel={
    ...
    ToolName.graphblast: ToolConfigurations(
        ...
        algo_rel={
            ...
            # Нет отдельного бинарника PR; путь дублирует gtc только для прохождения проверки сборки.
            AlgorithmName.pr:   Path('gtc'),
        },
        config=Namespace(
