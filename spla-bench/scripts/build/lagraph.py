...
def suitesparse_build() -> Tuple[Path, Path]:
    ...
    if not gb_build.exists():
        os.makedirs(gb_build, exist_ok=True)
...
def suitesparse_chosen_method() -> SuitesparseMethod:
    ...
    # local — только если заданы реальные пути (пустой Namespace из config не считается)
    gb_use_local = sp.local is not None and sp.local.include is not None and sp.local.library is not None
...
def build():
    ...
    # TOOL_CONFIG ожидает бинарники в deps/lagraph/build/..., а cmake пишет в build_<method>/
    link_path = LAGRAPH_PATHS.build.resolve()
    target = lagraph_build_dir.resolve()
